// import { prependOnceListener } from "process";
// import { Children } from "react";
import styles from "./Home.module.css";

const Home = () => {
  return (
    <main className={styles.home}>
      <div className={styles.rectangleDiv} />
      <img
        className={styles.aec84e30b961a315cbd3b2dffe09dcIcon}
        alt=""
        src="../5aec84e30b961a315cbd3b2dffe09dc7-1@2x.png"
      />
      <div className={styles.groupDiv}>
        <div className={styles.rectangleDiv1} />
        <div className={styles.rectangleDiv2} />
      </div>
      <img className={styles.vectorIcon} alt="" />
      <div className={styles.packageToursHeading}>
        <div className={styles.groupDiv1}>
          <div className={styles.rectangleDiv3} />
          <p className={styles.packageTours}>
            <span>{`Package `}</span>
            <span className={styles.tours}>Tours</span>
          </p>
        </div>
        <p className={styles.thisIsAUniqueExperienceOn}>
          This is a unique experience on ship, sea and land, exploring different
          coral islands of Lakshadweep.
        </p>
      </div>
      <div className={styles.pacakgeCad2}>
        <div className={styles.rectangleDiv4} />
        <img
          className={styles.rectangleIcon}
          alt=""
          src="../rectangle-24@2x.png"
        />
        <div className={styles.groupDiv2}>
          <div className={styles.rectangleDiv5} />
          <b className={styles.book}>Book</b>
        </div>
        <div className={styles.div}>
          <b className={styles.b}>{`₹25.0 `}</b>
        </div>
        <b className={styles.samudramPackage}>Samudram Package</b>
        <div
          className={styles.jikSkaratKabelBiobegon}
        >{`Jik säkarat. Kabel biobegon. `}</div>
        <div className={styles.groupDiv3}>
          <div className={styles.rectangleDiv6} />
          <div className={styles.groupDiv4}>
            <div className={styles.starDiv} />
            <b className={styles.b1}>4.5</b>
          </div>
        </div>
      </div>
      <div className={styles.pacakgeCad21}>
        <div className={styles.rectangleDiv4} />
        <img
          className={styles.rectangleIcon}
          alt=""
          src="../rectangle-24@2x.png"
        />
        <div className={styles.groupDiv5}>
          <div className={styles.rectangleDiv8} />
          <b className={styles.book}>Book</b>
        </div>
        <div className={styles.div1}>
          <b className={styles.b}>{`₹25.0 `}</b>
        </div>
        <b className={styles.samudramPackage1}>Samudram Package</b>
        <div
          className={styles.jikSkaratKabelBiobegon}
        >{`Jik säkarat. Kabel biobegon. `}</div>
        <div className={styles.groupDiv6}>
          <div className={styles.rectangleDiv6} />
          <div className={styles.groupDiv4}>
            <div className={styles.starDiv} />
            <b className={styles.b1}>4.5</b>
          </div>
        </div>
      </div>
      <div className={styles.pacakgeCad22}>
        <div className={styles.rectangleDiv4} />
        <img
          className={styles.rectangleIcon}
          alt=""
          src="../rectangle-242@2x.png"
        />
        <div className={styles.groupDiv5}>
          <div className={styles.rectangleDiv8} />
          <b className={styles.book}>Book</b>
        </div>
        <div className={styles.div1}>
          <b className={styles.b}>{`₹25.0 `}</b>
        </div>
        <b className={styles.samudramPackage2}>Samudram Package</b>
        <div
          className={styles.jikSkaratKabelBiobegon2}
        >{`Jik säkarat. Kabel biobegon. `}</div>
        <div className={styles.groupDiv6}>
          <div className={styles.rectangleDiv6} />
          <div className={styles.groupDiv4}>
            <div className={styles.starDiv} />
            <b className={styles.b1}>4.5</b>
          </div>
        </div>
      </div>
      <div className={styles.heroSection}>
        <div className={styles.rectangleDiv13} />
        <img
          className={styles.cardRight2Icon}
          alt=""
          src="../cardright2@2x.png"
        />
        <img
          className={styles.cardRight3Icon}
          alt=""
          src="../cardright3@2x.png"
        />
        <img className={styles.cardLeftIcon} alt="" src="../cardleft@2x.png" />
        <h1 className={styles.theExoticLakshadweepIslands}>
          <p className={styles.theExoticLakshadweep}>
            <span className={styles.theExotic}>{`The Exotic `}</span>
            <span>Lakshadweep</span>
          </p>
          <p className={styles.islands}>
            <span>Islands</span>
          </p>
        </h1>
        <img
          className={styles.cardRight1Icon}
          alt=""
          src="../cardright1@2x.png"
        />
        <form className={styles.locationFindTrip}>
          {/* <img className={styles.groupIcon} alt="" src="../group-10.svg" /> */}
          <form className={styles.locationFindTrip1}>
            <div className={styles.rectangleDiv14} />
            <div className={styles.lineDiv} />
            <div className={styles.frameDiv}>
              <form className={styles.groupDiv11}>
                <input
                  type="text"
                  name="name"
                  value="Pembroke Pines"
                  className={styles.pembrokePines}
                />
                <div className={styles.frameDiv1}>
                  <div className={styles.rectangleDiv15} />
                  <div className={styles.location}>Location</div>
                </div>
              </form>
              <form className={styles.groupDiv12}>
                <input
                  type="date"
                  name="date"
                  value="2017-01-26"
                  className={styles.december2022}
                />
                <div className={styles.frameDiv1}>
                  <div className={styles.rectangleDiv16}>
                    <div className={styles.rectangleDiv17} />
                  </div>
                  <div className={styles.location}>Date</div>
                </div>
              </form>
            </div>
            <div className={styles.groupDiv13}>
              <button className={styles.rectangleDiv18}>Find Trip</button>
              {/* <div className={styles.findTrip}>Find Trip</div> */}
            </div>
          </form>
        </form>
      </div>
      <div className={styles.groupDiv14}>
        <div className={styles.groupDiv15}>
          <div className={styles.groupDiv16}>
            <div className={styles.groupDiv17}>
              <div className={styles.rectangleDiv19} />
              <b className={styles.toursGallery}>
                <span>{`Tours `}</span>
                <span className={styles.tours}>Gallery</span>
              </b>
            </div>
            <div className={styles.seeTheScenicBeautyOfLaksh}>
              See the scenic beauty of Lakshadweep Islands which make them coral
              paradise of the world.
            </div>
          </div>
        </div>
        <div className={styles.groupDiv18}>
          <img className={styles.groupIcon1} alt="" src="../group-23.svg" />
          <img className={styles.groupIcon2} alt="" src="../group-23.svg" />
          <div className={styles.groupDiv19}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-29@2x.png"
            />
            <div className={styles.rectangleDiv20} />
            <div className={styles.frameDiv3}>
              <div className={styles.loremIpsum}>Lorem ipsum</div>
              <div className={styles.location1}>location</div>
            </div>
          </div>
          <div className={styles.groupDiv20}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-33@2x.png"
            />
            <div className={styles.groupDiv21}>
              <div className={styles.rectangleDiv21} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv22}>
            <img
              className={styles.rectangleIcon5}
              alt=""
              src="../rectangle-32@2x.png"
            />
            <div className={styles.groupDiv23}>
              <div className={styles.rectangleDiv22} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv24}>
            <img
              className={styles.rectangleIcon5}
              alt=""
              src="../rectangle-35@2x.png"
            />
            <div className={styles.groupDiv25}>
              <div className={styles.rectangleDiv22} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv26}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-36@2x.png"
            />
            <div className={styles.groupDiv21}>
              <div className={styles.rectangleDiv21} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv28}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-37@2x.png"
            />
            <div className={styles.groupDiv21}>
              <div className={styles.rectangleDiv21} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv30}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-34@2x.png"
            />
            <div className={styles.groupDiv31}>
              <div className={styles.rectangleDiv21} />
              <div className={styles.frameDiv4}>
                <div className={styles.loremIpsum}>Lorem ipsum</div>
                <div className={styles.location1}>location</div>
              </div>
            </div>
          </div>
          <div className={styles.groupDiv32}>
            <img
              className={styles.rectangleIcon5}
              alt=""
              src="../rectangle-31@2x.png"
            />
            <div className={styles.rectangleDiv27} />
            <div className={styles.frameDiv10}>
              <div className={styles.loremIpsum}>Lorem ipsum</div>
              <div className={styles.location1}>location</div>
            </div>
          </div>
          <div className={styles.groupDiv33}>
            <img
              className={styles.rectangleIcon3}
              alt=""
              src="../rectangle-30@2x.png"
            />
            <div className={styles.rectangleDiv20} />
            <div className={styles.frameDiv3}>
              <div className={styles.loremIpsum}>Lorem ipsum</div>
              <div className={styles.location1}>location</div>
            </div>
          </div>
        </div>
      </div>
      <img
        className={styles.rectangleIcon12}
        alt=""
        src="../rectangle-41@2x.png"
      />
      <div className={styles.rectangleDiv29} />
      <div className={styles.frameDiv12}>
        <div className={styles.selectIsaland}>select isaland</div>
        <img
          className={styles.expandMoreFILL0Wght400GRADIcon}
          alt=""
          src="../expand-more-fill0-wght400-grad0-opsz48-1.svg"
        />
      </div>
      <div className={styles.discoverTheMostEngagingHol}>
        Discover the most engaging Holidays
      </div>
      <div className={styles.chooseFromTheVariousPackag}>
        Choose from the various Package Tours and Island Stays which suits you.
      </div>
      <div className={styles.rectangleDiv30} />
      <div className={styles.rectangleDiv31} />
      <div className={styles.frameDiv13}>
        <img
          className={styles.exploreFILL0Wght400GRAD0OpIcon}
          alt=""
          src="../explore-fill0-wght400-grad0-opsz48-1.svg"
        />
        <div className={styles.selectIsaland}>explore</div>
      </div>
      <div className={styles.frameDiv14}>
        <div className={styles.rectangleDiv32} />
        <div className={styles.loremIpsum}>Visit</div>
      </div>
      <div className={styles.frameDiv15}>
        <div className={styles.rectangleDiv32} />
        <h3 className={styles.whyGOL}>
          <span className={styles.why}>{`why `}</span>
          <b className={styles.gOL}>GOL?</b>
        </h3>
      </div>
      <p
        className={styles.goLGoLakshadweepIsAGove}
      >{`GoL (Go Lakshadweep) is a Government Authorized Travel Agent (ATA) of Lakshadweep Tourism Department.GoL bridges a gap in the market that poses a great deal of difficulty, uncertainty and lack of authentic information for tourists to the U.T. of Lakshadweep in India Traveling to Lakshadweep as a tourist is needed to have certain procedures because of its special rules & regulations and geographical differences. This could be a major cause of worry many to visit this paradise on earth. GoL makes it easy for you thorough this platform. You can trust us in a friendly manner. Gol is run and manned by native Lakshadweep Islanders so we can provide exact information with safe and secure more than any other platform available. ...`}</p>
      <div className={styles.frameDiv16}>
        <button className={styles.readMore}>read more</button>
      </div>
      <img className={styles.groupIcon3} alt="" src="../group-34@2x.png" />
      <p className={styles.seeDo}>{`see & do`}</p>
      <img className={styles.vectorIcon1} alt="" src="../vector-5.svg" />
      <div className={styles.frameDiv17}>
        <div className={styles.frameDiv18}>
          <div className={styles.frameDiv19}>
            <img className={styles.image1Icon} alt="" src="../image-1@2x.png" />
            <div className={styles.rectangleDiv34} />
            <div className={styles.groupDiv34}>
              <div className={styles.groupDiv35}>
                <p className={styles.trustworthyBookingPlatform}>
                  <p className={styles.theExoticLakshadweep}>
                    Trustworthy Booking
                  </p>
                  <p className={styles.platform}>Platform</p>
                </p>
              </div>
            </div>
          </div>
          <div className={styles.frameDiv19}>
            <img
              className={styles.image1Icon}
              alt=""
              src="../image-11@2x.png"
            />
            <div className={styles.rectangleDiv35} />
            <div className={styles.groupDiv36}>
              <div className={styles.groupDiv37}>
                <p className={styles.trustworthyBookingPlatform}>
                  <p className={styles.theExoticLakshadweep}>Easy Travel</p>
                  <p className={styles.platform}>Bookings</p>
                </p>
              </div>
            </div>
          </div>
          <div className={styles.frameDiv19}>
            <img
              className={styles.image1Icon}
              alt=""
              src="../image-12@2x.png"
            />
            <div className={styles.rectangleDiv36} />
            <div className={styles.groupDiv38}>
              <div className={styles.groupDiv39}>
                <p className={styles.trustworthyBookingPlatform}>
                  <p className={styles.theExoticLakshadweep}>Various Holiday</p>
                  <p className={styles.platform}>Options</p>
                </p>
              </div>
            </div>
          </div>
          <div className={styles.frameDiv19}>
            <img
              className={styles.image1Icon}
              alt=""
              src="../image-13@2x.png"
            />
            <div className={styles.rectangleDiv37} />
            <div className={styles.groupDiv40}>
              <div className={styles.groupDiv41}>
                <p className={styles.trustworthyBookingPlatform}>
                  <p className={styles.theExoticLakshadweep}>GoL Supportin</p>
                  <p className={styles.platform}>Islands</p>
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className={styles.groupDiv42}>
        <div className={styles.rectangleDiv38} />
        <h3 className={styles.attractions}>Attractions</h3>
      </div>
      <p className={styles.indulgeInTheManyWaterSpor}>
        <p className={styles.theExoticLakshadweep}>
          Indulge in the many water sport activities offered by almost all of
          the resorts in the islands!If your property
        </p>
        <p className={styles.platform}>
          does not have such attractions,there are numerous independent water
          sport providers. Explore what each of these attractions are about!
        </p>
      </p>
      <div className={styles.groupDiv43}>
        <div className={styles.rectangleDiv39} />
        <p className={styles.packageTours}>
          <span>{`Island `}</span>
          <span className={styles.tours}>stay</span>
        </p>
      </div>
      <p className={styles.seeTheScenicBeautyOfLaksh1}>
        See the scenic beauty of Lakshadweep Islands which make them coral
        paradise of the world.
      </p>
      <div className={styles.frameDiv23}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-47@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <p className={styles.thinnakkara}>Thinnakkara</p>
              <button className={styles.frameButton}>
                <button className={styles.readMore1}>read more</button>
              </button>
            </div>
            <p className={styles.thinnakaraIsOneOfTheMany}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </p>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv27}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-471@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv32}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-472@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv37}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-473@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <p className={styles.thinnakkara}>Thinnakkara</p>
              <button className={styles.frameButton}>
                <div className={styles.readMore4}>read more</div>
              </button>
            </div>
            <p className={styles.thinnakaraIsOneOfTheMany}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </p>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv41}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-474@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv46}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-475@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv51}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-474@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv56}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-477@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv61}>
        <img
          className={styles.rectangleIcon21}
          alt=""
          src="../rectangle-478@2x.png"
        />
        <div className={styles.frameDiv62}>
          <div className={styles.frameDiv63}>
            <div className={styles.frameDiv64}>
              <p className={styles.thinnakkara}>Thinnakkara</p>
              <button className={styles.frameButton2}>
                <div className={styles.readMore4}>read more</div>
              </button>
            </div>
            <p className={styles.thinnakaraIsOneOfTheMany}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </p>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv65}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-479@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <p className={styles.thinnakkara}>Thinnakkara</p>
              <button className={styles.frameButton}>
                <div className={styles.readMore4}>read more</div>
              </button>
            </div>
            <p className={styles.thinnakaraIsOneOfTheMany}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </p>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv69}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-4710@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv74}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-4710@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <p className={styles.thinnakkara}>Thinnakkara</p>
              <button className={styles.frameButton}>
                <div className={styles.readMore4}>read more</div>
              </button>
            </div>
            <p className={styles.thinnakaraIsOneOfTheMany}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </p>
          </div>
        </div>
      </div>
      <div className={styles.frameDiv78}>
        <img
          className={styles.rectangleIcon13}
          alt=""
          src="../rectangle-475@2x.png"
        />
        <div className={styles.frameDiv24}>
          <div className={styles.frameDiv25}>
            <div className={styles.frameDiv26}>
              <b className={styles.thinnakkara1}>Thinnakkara</b>
              <div className={styles.frameDiv31}>
                <div className={styles.selectIsaland}>read more</div>
              </div>
            </div>
            <div className={styles.thinnakaraIsOneOfTheMany1}>
              Thinnakara is one of the many uninhabited islands in the Arabian
              Sea, this Island is a tiny has managed to retain its exquisite
              natural
            </div>
          </div>
        </div>
      </div>
      <button className={styles.rectangleButton} />
      <div className={styles.groupDiv44}>
        <div className={styles.groupDiv45}>
          <img
            className={styles.favoriteFILL0Wght400GRAD0OIcon}
            alt=""
            src="../favorite-fill0-wght400-grad0-opsz48-1.svg"
          />
          <div className={styles.romance}>Romance</div>
        </div>
      </div>
      <button className={styles.rectangleButton1} />
      <div className={styles.groupDiv46}>
        <img
          className={styles.favoriteFILL0Wght400GRAD0OIcon}
          alt=""
          src="../beach-access-fill0-wght400-grad0-opsz48-1.svg"
        />
        <div className={styles.romance}>Beach</div>
      </div>
      <button className={styles.rectangleButton2} />
      <div className={styles.groupDiv47}>
        <img
          className={styles.favoriteFILL0Wght400GRAD0OIcon}
          alt=""
          src="../beach-access-fill0-wght400-grad0-opsz48-1.svg"
        />
        <div className={styles.romance}>City</div>
      </div>
      <button className={styles.rectangleButton3} />
      <div className={styles.groupDiv48}>
        <img
          className={styles.favoriteFILL0Wght400GRAD0OIcon}
          alt=""
          src="../directions-bike-fill0-wght400-grad0-opsz48-1.svg"
        />
        <div className={styles.romance}>Outdoors</div>
      </div>
      <header className={styles.header}>
        <nav className={styles.groupNav}>
          <img
            className={styles.golLogo1Icon}
            alt=""
            src="../gollogo-1@2x.png"
          />
          <div className={styles.groupDiv49}>
            <div className={styles.fINDRESERVATIONS}>FIND RESERVATIONS</div>
            <div className={styles.pACKAGE}>PACKAGE</div>
            <div className={styles.iSLANDSTAYS}>ISLAND STAYS</div>
            <div className={styles.aBOUTLAKSHADWEEP}>ABOUT LAKSHADWEEP</div>
            <div className={styles.hELP}>HELP</div>
          </div>
          <div className={styles.loginBtn}>
            <div className={styles.rectangleDiv40} />
            <b className={styles.loginSignup}>Login / Signup</b>
          </div>
        </nav>
        <div className={styles.header1} />
      </header>
    </main>
  );
};

export default Home;
